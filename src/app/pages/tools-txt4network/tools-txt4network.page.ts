...
@Component({
  selector: 'app-tools-txt4network-page',
  imports: [CommonModule, FormsModule, BreadcrumbsComponent],
  templateUrl: './tools-txt4network.page.html',
  styleUrl: './tools-txt4network.page.scss',
})
export class ToolsTxt4networkPage {
  ...
  analyze(): void {
    ...
    this.error = '';
    this.http
      .post('/api/tools/txt4network', { text }, {
      ...
      .subscribe({
        ...
        error: (error: HttpErrorResponse) => {
          this.error = error.status === 401 || error.status === 403
            ...
            : 'Não foi possível analisar a rede. Tente novamente.';
        },
      });
  }
  ...
  private downloadResult(response: HttpResponse<Blob>): void {
    if (!response.body) {
      this.error = 'A API não retornou o arquivo da análise.';
      return;
    }
