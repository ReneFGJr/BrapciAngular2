...
var AuthService = class _AuthService {
  ...
  persistUser(user, renewSession = false) {
  ...
  writeUserToLocalStorage(user, renewSession) {
  ...
  extractUserFromLoginResponse(response) {
    ...
    if (authResponse.user && typeof authResponse.user === "object") {
      ...
      return __spreadProps(__spreadValues({}, user), { token: user.token || this.extractToken(authResponse) || void 0 });
    }
  ...
  checkSession() {
    const localUser = this.readUserFromLocalStorage();
    const verification = localUser?.token ? this.loginOauthHttp(localUser.token).pipe(map((response) => {
    ...
    })) : this.http.get(this.buildAuthUrl("/auth/me")).pipe(map((response) => response.user));
    return verification.pipe(map((user) => user && localUser?.themePreference ? __spreadProps(__spreadValues({}, user), { themePreference: localUser.themePreference }) : user), tap((user) => this.persistUser(user)), catchError((error) => {
  ...
  getApiToken() {
    ...
    if (!cached)
      return "";
    try {
      return this.extractToken(JSON.parse(cached));
    } catch {
      return "";
    }
  }
...
//# sourceMappingURL=chunk-LP2Y7HO2.mjs.map
