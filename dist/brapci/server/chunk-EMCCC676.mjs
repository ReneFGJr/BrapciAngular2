...
import {
  BrapciApiService
} from "./chunk-TKUEJ4DK.mjs";
import {
  BreadcrumbsComponent
} from "./chunk-SDW3XDIK.mjs";
import "./chunk-DZQKGNLQ.mjs";
import {
...
import "./chunk-MLMAXTOA.mjs";
import "./chunk-NWKFZVVO.mjs";
import {
  CommonModule,
  Component,
  EventEmitter,
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(UploadFileComponent, [{
    type: Component,
    args: [{ selector: "app-upload-file", standalone: true, imports: [ReactiveFormsModule, FormFileInputComponent, LoadingSpinnerComponent], template: `<div class="col-lg-6 col-12">
  <section class="livros-submit-card h-100" aria-labelledby="upload-title">
    ...
    @if (status === 1 && dataset) {
      <div class="upload-process">
        ...
        @if (emailSend) {
          <div class="pt-3 pb-2">
            <span class="field-hint">Confirma\xE7\xE3o de autoria</span>
            <form class="mt-2" [formGroup]="emailForm" (ngSubmit)="onSubmitEmail()">
              <div class="mb-3">
                ...
                @if (emailForm.controls.email.invalid && emailForm.controls.email.touched) {
                  <div class="text-danger">Por favor, insira um e-mail v\xE1lido.</div>
                }

                <label for="agree" class="form-check-label mt-3">
                  <input id="agree" type="checkbox" class="form-check-input me-2" formControlName="agree" />
                  Concordo com os termos
                </label>
              </div>
              <button type="submit" class="btn btn-primary full" [disabled]="emailForm.invalid">
                Enviar
              </button>
            </form>
          </div>
        } @else {
          <button type="button" class="btn btn-primary full mt-3" (click)="process()">
            {{ buttonName }}
          </button>
        }
      </div>
    ...
    @if (status === 3 && dataset) {
      <div class="upload-process">
        ...
        <p>
          A submiss\xE3o de seu livro
          <span [title]="dataset['file']">{{ displayFileName(dataset['file']) }}</span>
          foi realizada com sucesso!
        </p>
        <p>Um e-mail foi enviado para sua conta, autorizando a publica\xE7\xE3o.</p>
        <div class="alert alert-success mt-2">
          <strong>Sucesso:</strong> Seu livro foi enviado com \xEAxito. Assim que recebermos o retorno
          do e-mail (<strong>{{ dataset['email'] }}</strong>) de confirma\xE7\xE3o, iniciaremos o processo
          de indexa\xE7\xE3o.
        </div>
        <div class="alert alert-warning mt-2">
          <strong>Aten\xE7\xE3o:</strong> Caso n\xE3o encontre o e-mail em sua caixa de entrada, verifique a
          caixa de spam.
        </div>
...
//# sourceMappingURL=chunk-EMCCC676.mjs.map
