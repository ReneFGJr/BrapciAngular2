...
var AuthService = class _AuthService {
  ...
  checkSession() {
    return this.http.get(this.buildAuthUrl("/auth/me")).pipe(map((response) => {
      if (!response.user || response.user.token)
        return response.user;
      const preservedToken = this.userSubject.value?.token || this.readUserFromLocalStorage()?.token;
      return preservedToken ? __spreadProps(__spreadValues({}, response.user), { token: preservedToken }) : response.user;
    }), tap((user) => {
...
//# sourceMappingURL=chunk-KNBI7DSD.mjs.map
