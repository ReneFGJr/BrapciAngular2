...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(PainelAnalysisComponent, [{
    type: Component,
    args: [{ selector: "app-painel-analysis", standalone: true, imports: [CommonModule, BarChartComponent, TranslateModule], template: `<div class="painel-analysis">\r
  <h1 class="h3 mb-3">{{ 'painelAnalysis.productionIndicators' | translate }}</h1>\r
\r
  <div class="card border-0 shadow-sm mb-3">\r
    <div class="card-body">\r
      <div class="d-flex align-items-center justify-content-between gap-2 mb-3">\r
        <h3 class="h6 mb-0">{{ 'painelAnalysis.productionByYear' | translate }}</h3>\r
        <button type="button" class="btn btn-sm btn-outline-secondary" [disabled]="!yearsChartPoints().length" (click)="exportYearsCsv()">{{ 'painelAnalysis.exportCsv' | translate }}</button>\r
      </div>\r
      <app-bar-chart title="" [points]="yearsChartPoints()"></app-bar-chart>\r
    </div>\r
  </div>\r
\r
  <div class="row g-3">\r
    @for (section of tableSections(); track section.key) {\r
      <div class="col-12 col-lg-6">\r
        <div class="card border-0 shadow-sm h-100">\r
          <div class="card-body">\r
            <div class="d-flex align-items-center justify-content-between gap-2 mb-3">\r
              <h3 class="h6 mb-0">{{ section.titleKey | translate }}</h3>\r
              <button type="button" class="btn btn-sm btn-outline-secondary" [disabled]="!section.rows.length" (click)="exportSectionCsv(section)">{{ 'painelAnalysis.exportCsv' | translate }}</button>\r
            </div>\r
            @if (section.rows.length) {\r
              <div class="table-responsive">\r
                <table class="table table-sm align-middle mb-0">\r
                  <thead><tr><th>{{ 'painelAnalysis.item' | translate }}</th><th class="text-end">{{ 'painelAnalysis.value' | translate }}</th></tr></thead>\r
                  <tbody>\r
                    @for (row of section.rows; track row.label) {\r
                      <tr><td>{{ row.label }}</td><td class="text-end fw-semibold">{{ row.value }}</td></tr>\r
                    }\r
                  </tbody>\r
                </table>\r
              </div>\r
            } @else {\r
              <p class="text-muted mb-0">{{ 'painelAnalysis.noData' | translate }}</p>\r
            }\r
          </div>\r
        </div>\r
      </div>\r
...
\r
  @if (networkMetrics().length || networkAuthors().length) {\r
    <h1 class="h3 mt-4 mb-3">{{ 'painelAnalysis.connectionIndicators' | translate }}</h1>\r
    <div class="card border-0 shadow-sm mb-3">\r
      <div class="card-body">\r
        <div class="d-flex align-items-center justify-content-between gap-2 mb-3">\r
          <h3 class="h6 mb-0">{{ 'painelAnalysis.network.title' | translate }}</h3>\r
          <button type="button" class="btn btn-sm btn-outline-secondary" [disabled]="!networkAuthors().length" (click)="exportNetworkAuthorsCsv()">{{ 'painelAnalysis.exportCsv' | translate }}</button>\r
        </div>\r
        @if (networkMetrics().length) {\r
          <div class="row g-2 mb-3">\r
            @for (metric of networkMetrics(); track metric.key) {\r
              <div class="col-6 col-lg-3"><div class="network-metric h-100"><span>{{ metric.labelKey | translate }}</span><strong>{{ metric.value }}</strong></div></div>\r
            }\r
          </div>\r
        }\r
        @if (networkAuthors().length) {\r
          <div class="table-responsive network-table">\r
            <table class="table table-sm align-middle mb-0">\r
              <thead><tr>\r
                <th>{{ 'painelAnalysis.network.author' | translate }}</th>\r
                <th class="text-end">{{ 'painelAnalysis.network.indicators.degree' | translate }}</th>\r
                <th class="text-end">{{ 'painelAnalysis.network.indicators.weightedDegree' | translate }}</th>\r
                <th class="text-end">{{ 'painelAnalysis.network.indicators.betweenness' | translate }}</th>\r
                <th class="text-end">{{ 'painelAnalysis.network.indicators.closeness' | translate }}</th>\r
                <th class="text-end">{{ 'painelAnalysis.network.indicators.eigenvector' | translate }}</th>\r
                <th class="text-end">{{ 'painelAnalysis.network.indicators.community' | translate }}</th>\r
              </tr></thead>\r
              <tbody>\r
                @for (author of networkAuthors(); track author.author) {\r
                  <tr><td>{{ author.author }}</td><td class="text-end">{{ author.degree }}</td><td class="text-end">{{ author.weightedDegree }}</td><td class="text-end">{{ author.betweenness }}</td><td class="text-end">{{ author.closeness }}</td><td class="text-end">{{ author.eigenvector }}</td><td class="text-end">{{ author.community }}</td></tr>\r
                }\r
              </tbody>\r
            </table>\r
          </div>\r
          <div class="network-legend mt-3" [attr.aria-label]="'painelAnalysis.network.legendTitle' | translate">\r
            <h4 class="h6 mb-2">{{ 'painelAnalysis.network.legendTitle' | translate }}</h4>\r
            <dl class="mb-0">\r
              @for (metric of ['nodes', 'edges', 'density', 'modularity']; track metric) {\r
                <div><dt>{{ 'painelAnalysis.network.metrics.' + metric | translate }}</dt><dd>{{ 'painelAnalysis.network.metricDescriptions.' + metric | translate }}</dd></div>\r
              }\r
              @for (indicator of ['degree', 'weightedDegree', 'betweenness', 'closeness', 'eigenvector', 'community']; track indicator) {\r
                <div><dt>{{ 'painelAnalysis.network.indicators.' + indicator | translate }}</dt><dd>{{ 'painelAnalysis.network.descriptions.' + indicator | translate }}</dd></div>\r
              }\r
            </dl>\r
          </div>\r
        }\r
...
//# sourceMappingURL=chunk-R57XSLEF.mjs.map
