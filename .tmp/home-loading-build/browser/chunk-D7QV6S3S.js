...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(ToolsTxt4networkPage, [{
    type: Component,
    args: [{ selector: "app-tools-txt4network-page", imports: [CommonModule, FormsModule, BreadcrumbsComponent], template: `<section class="txt4network-wrap py-4" aria-labelledby="txt4network-title">\r
  <div class="container">\r
    <app-breadcrumbs [labels]="{ tools: 'Ferramentas', txt4network: 'An\xE1lise de Rede' }"></app-breadcrumbs>\r
\r
    <div class="txt4network-card p-4 p-md-5">\r
      <h1 id="txt4network-title" class="h2 mb-2">An\xE1lise de Rede</h1>\r
      <p class="txt4network-subtitle mb-4">\r
        Insira os dados da rede ou selecione um arquivo para gerar os indicadores de colabora\xE7\xE3o entre pesquisadores.\r
      </p>\r
\r
      <div class="row g-4 align-items-start">\r
        <div class="col-12 col-lg-7">\r
          <form (ngSubmit)="analyze()">\r
            <div class="mb-3">\r
              <label for="network-text" class="form-label fw-semibold">Dados da rede</label>\r
              <textarea id="network-text" name="network" class="form-control" rows="8" [(ngModel)]="text" [disabled]="loading" placeholder="Cole aqui os dados da rede" required></textarea>\r
              <div class="form-text">Informe os dados que ser\xE3o utilizados para calcular os indicadores da rede.</div>\r
            </div>\r
\r
            <div class="mb-3">\r
              <label for="network-file" class="form-label fw-semibold">Carregar arquivo</label>\r
              <input id="network-file" class="form-control" type="file" accept=".txt,.net,text/plain" [disabled]="loading" (change)="loadFile($event)" />\r
              @if (selectedFileName) { <div class="form-text">Arquivo carregado: {{ selectedFileName }}</div> }\r
            </div>\r
\r
            @if (error) { <div class="alert alert-danger" role="alert">{{ error }}</div> }\r
\r
            <button type="submit" class="btn btn-primary" [disabled]="!text.trim() || loading">\r
              @if (loading) { <span class="spinner-border spinner-border-sm me-2" aria-hidden="true"></span> Analisando... }\r
              @else { Analisar rede }\r
            </button>\r
          </form>\r
        </div>\r
\r
        <aside class="col-12 col-lg-5" aria-labelledby="network-instructions-title">\r
          <h2 id="network-instructions-title" class="h5 mb-3">Como utilizar</h2>\r
          <div class="instructions-card p-3">\r
            <ol class="mb-0 ps-3">\r
              <li>Insira os dados no campo ou carregue um arquivo.</li>\r
              <li>Confira o conte\xFAdo antes de iniciar.</li>\r
              <li>Clique em \u201CAnalisar rede\u201D para baixar o resultado.</li>\r
            </ol>\r
          </div>\r
        </aside>\r
      </div>\r
...
//# sourceMappingURL=chunk-D7QV6S3S.js.map
