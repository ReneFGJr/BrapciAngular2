...
\r
      @if (activeTab() === 'overview') {\r
        ...
        <section class="overview-pies mt-4" aria-label="Distribui\xE7\xE3o das revistas">\r
          @for (chart of overviewPieCharts(); track chart.title) {\r
            <article class="content-card pie-card">\r
              <h2 class="h5 mb-3">{{ chart.title }}</h2>\r
              <div class="pie-layout">\r
                <div class="pie-chart" role="img" [attr.aria-label]="chart.title + ': ' + chart.total + ' revistas'" [style.background]="chart.background"><span>{{ chart.total }}</span></div>\r
                <ul class="pie-legend">\r
                  @for (slice of chart.slices; track slice.label) {\r
                    <li><span class="pie-swatch" [style.background]="slice.color" aria-hidden="true"></span><span class="pie-label">{{ slice.label }}</span><strong>{{ slice.value }}</strong><small>{{ slice.percentage | number:'1.0-1' }}%</small></li>\r
                  }\r
                </ul>\r
              </div>\r
            </article>\r
...
//# sourceMappingURL=chunk-XGLNAZEX.js.map
