...
(() => {
  ...
  `, styles: ["/* angular:styles/component:scss;b303de315785e790c7a0ae10123983ed93e68318c5fc4e22b1e4c8cb8f23da51;D:/www/BrapciAngular2/src/app/pages/admin-edit/admin-concept-editor.component.ts */\n.autocomplete-query {\n  margin: 0 0 0.5rem;\n  padding: 0.65rem 0.8rem;\n  border: 1px solid var(--bs-border-color);\n  border-radius: 0.4rem;\n  background: var(--bs-tertiary-bg);\n  color: var(--bs-body-color);\n  font-size: 0.78rem;\n  white-space: pre-wrap;\n}\n/*# sourceMappingURL=admin-concept-editor.component.css.map */\n"] }]
  }], null, { draft: [{
    type: Input,
    args: [{ required: true }]
  }], query: [{
...
\r
          <ng-container *ngFor="let data of property.Data; let dataIndex = index">\r
            <div *ngIf="dataIndex === 0 || property.Data[dataIndex - 1].n_lang !== data.n_lang" class="rdf-language-separator">\r
              <span>{{ languageLabel(data.n_lang) }}</span>\r
            </div>\r
            <article class="rdf-value">\r
              <div class="rdf-value__actions">\r
                <button type="button" class="rdf-icon rdf-icon--delete" [attr.aria-label]="'adminEdit.delete' | translate" [attr.title]="'adminEdit.delete' | translate" (click)="requestDelete(groupIndex, propertyIndex, dataIndex)">\r
                  <i class="bi bi-trash" aria-hidden="true"></i>\r
                </button>\r
                <button *ngIf="canEditData(property, data)" type="button" class="rdf-icon rdf-icon--edit" [attr.aria-label]="'adminEdit.edit' | translate" [attr.title]="'adminEdit.edit' | translate" (click)="openEdit(groupIndex, propertyIndex, dataIndex)">\r
                  <i class="bi bi-pencil" aria-hidden="true"></i>\r
                </button>\r
              </div>\r
              <div class="rdf-value__content">\r
                <span>{{ data.n_name || ('adminEdit.unnamed' | translate) }}</span>\r
                <sup *ngIf="data.n_lang">{{ data.n_lang }}</sup>\r
              </div>\r
            </article>\r
          </ng-container>\r
        </div>\r
...
<div *ngIf="editor() as state" class="admin-modal" role="dialog" aria-modal="true" [attr.aria-label]="'adminEdit.modalTitle' | translate">\r
  ...
  <section class="admin-modal__panel">\r
    <header><div><span class="admin-edit__eyebrow">{{ state.draft.c_class }}</span><h2>{{ (state.dataIndex === null ? 'adminEdit.addTitle' : 'adminEdit.editTitle') | translate }}</h2></div><button type="button" class="btn-close" [attr.aria-label]="'adminEdit.close' | translate" (click)="closeEditor()"></button></header>\r
    <div class="admin-modal__body">\r
      <app-admin-image-editor *ngIf="state.mode === 'image' || state.mode === 'file'; else valueEditor" [mode]="state.mode === 'image' ? 'image' : 'file'" [draft]="state.draft" (fileChange)="selectFile($event)" />\r
      <ng-template #valueEditor>\r
        <app-admin-literal-editor *ngIf="isLiteralDraft(state.draft); else conceptEditor" [draft]="state.draft" (change)="updateDraft($event.field, $event.value)" />\r
        <ng-template #conceptEditor>\r
          <app-admin-concept-editor\r
            [draft]="state.draft"\r
            [query]="autocompleteQuery()"\r
            [options]="autocompleteOptions()"\r
            [loading]="autocompleteLoading()"\r
            [payloadJson]="state.dataIndex === null ? '' : autocompletePayloadJson()"\r
            (queryChange)="updateAutocompleteQuery($event)"\r
            (selectionChange)="selectAutocomplete($event)"\r
            (classChange)="updateDraft('c_class', $event)"\r
          />\r
        </ng-template>\r
      </ng-template>\r
      <div *ngIf="state.dataIndex !== null" class="admin-modal__parameters">\r
        <label class="form-label" for="rdf-parameters">JSON</label>\r
        <pre id="rdf-parameters" aria-live="polite">{{ editorParameters(state.draft) }}</pre>\r
      </div>\r
      <div *ngIf="saveResponse() !== null" class="admin-modal__parameters">\r
        <div id="rdf-save-response" class="alert alert-secondary mb-0" aria-live="polite">\r
          <p class="mb-1"><strong>Status:</strong> {{ saveResponseStatus() }}</p>\r
          <p class="mb-0"><strong>Mensagem:</strong> {{ saveResponseMessage() }}</p>\r
        </div>\r
      </div>\r
    </div>\r
    <footer><button type="button" class="btn btn-outline-secondary" (click)="closeEditor()">{{ 'adminEdit.cancel' | translate }}</button><button type="button" class="btn btn-warning" [disabled]="uploading()" (click)="confirmEditor()">{{ (saveResponse() !== null ? 'adminEdit.close' : (uploading() ? 'adminEdit.uploading' : 'adminEdit.confirm')) | translate }}</button></footer>\r
  </section>\r
</div>\r
\r
<div *ngIf="deleteTarget() as target" class="admin-modal" role="alertdialog" aria-modal="true">\r
  ...
  <section class="admin-modal__panel admin-modal__panel--small">\r
    <header><h2>{{ 'adminEdit.deleteTitle' | translate }}</h2><button type="button" class="btn-close" (click)="cancelDelete()"></button></header>\r
    <div class="admin-modal__body">\r
      <p>{{ 'adminEdit.deleteConfirm' | translate: { name: target.data.n_name } }}</p>\r
    </div>\r
    <footer><button type="button" class="btn btn-outline-secondary" (click)="cancelDelete()">Fechar</button><button type="button" class="btn btn-danger" [disabled]="deleting()" (click)="confirmDelete()">{{ deleting() ? 'Excluindo...' : 'Excluir' }}</button></footer>\r
  </section>\r
</div>\r
...
//# sourceMappingURL=chunk-NGMQGD6G.js.map
