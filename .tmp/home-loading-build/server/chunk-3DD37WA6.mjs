...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(MonitorPage, [{
    type: Component,
    args: [{ selector: "app-monitor-page", standalone: true, imports: [CommonModule, RouterLink, TranslateModule], template: `<main class="monitor-page">\r
  <section class="monitor-hero">\r
    <div class="container">\r
      <a class="monitor-back" routerLink="/"><i class="bi bi-arrow-left" aria-hidden="true"></i> Brapci</a>\r
      <div class="monitor-heading">\r
        <div>\r
          <p class="monitor-kicker">{{ 'monitor.kicker' | translate }}</p>\r
          <h1>{{ 'monitor.title' | translate }}</h1>\r
          <p class="monitor-description">{{ 'monitor.description' | translate }}</p>\r
        </div>\r
        <button type="button" class="refresh-button" [disabled]="refreshing()" (click)="loadComputers()">\r
          <i class="bi bi-arrow-clockwise" [class.spinning]="refreshing()" aria-hidden="true"></i>\r
          {{ 'monitor.refresh' | translate }}\r
        </button>\r
      </div>\r
    </div>\r
  </section>\r
\r
  <section class="container monitor-content" aria-live="polite">\r
    @if (loading()) {\r
      <div class="loading-heading">\r
        <div><p class="section-kicker">{{ 'monitor.availableNow' | translate }}</p><h2>{{ 'monitor.onlineComputers' | translate }}</h2></div>\r
        <span class="loading-indicator"><i class="bi bi-arrow-clockwise spinning" aria-hidden="true"></i> {{ 'monitor.loading' | translate }}</span>\r
      </div>\r
      <div class="computer-grid" [attr.aria-label]="'monitor.loading' | translate">\r
        @for (placeholder of [1, 2, 3, 4, 5, 6]; track placeholder) {\r
          <article class="computer-card computer-card--loading" aria-hidden="true">\r
            <span class="skeleton skeleton-icon"></span>\r
            <span class="skeleton-lines"><i></i><i></i></span>\r
            <span class="skeleton skeleton-badge"></span>\r
          </article>\r
        }\r
      </div>\r
    } @else if (error()) {\r
      <div class="monitor-state monitor-state--error"><i class="bi bi-exclamation-triangle" aria-hidden="true"></i><p>{{ error() | translate }}</p><button class="btn btn-primary" (click)="loadComputers()">{{ 'monitor.tryAgain' | translate }}</button></div>\r
    } @else {\r
      <div class="monitor-summary">\r
        <article class="summary-card summary-card--online"><span><i class="bi bi-check-circle-fill"></i> {{ 'monitor.online' | translate }}</span><strong>{{ online().length }}</strong></article>\r
        <article class="summary-card summary-card--offline"><span><i class="bi bi-x-circle-fill"></i> {{ 'monitor.offline' | translate }}</span><strong>{{ offline().length }}</strong></article>\r
        <article class="summary-card"><span><i class="bi bi-pc-display"></i> {{ 'monitor.total' | translate }}</span><strong>{{ computers().length }}</strong></article>\r
      </div>\r
\r
      <div class="section-heading"><div><p class="section-kicker">{{ 'monitor.availableNow' | translate }}</p><h2>{{ 'monitor.onlineComputers' | translate }}</h2></div>@if (updatedAt()) { <small>{{ 'monitor.updatedAt' | translate: { time: (updatedAt() | date:'HH:mm:ss') } }}</small> }</div>\r
      @if (online().length) {\r
        <div class="computer-grid">\r
          @for (computer of online(); track computer.ip) {\r
            <article class="computer-card computer-card--online">\r
              <div class="computer-icon"><i class="bi bi-pc-display-horizontal" aria-hidden="true"></i><span class="status-dot"></span></div>\r
              <div><h3>{{ computer.server }}</h3><p><i class="bi bi-hdd-network"></i> {{ computer.ip }}</p></div>\r
              <span class="status-badge">{{ 'monitor.onlineStatus' | translate }}</span>\r
            </article>\r
          }\r
        </div>\r
      } @else { <div class="empty-online"><i class="bi bi-wifi-off"></i><p>{{ 'monitor.noneOnline' | translate }}</p></div> }\r
\r
      @if (offline().length) {\r
        <details class="offline-section" open>\r
          <summary>{{ 'monitor.showOffline' | translate }} <span>{{ offline().length }}</span></summary>\r
          <div class="computer-grid mt-3">\r
            @for (computer of offline(); track computer.ip) {\r
              <article class="computer-card computer-card--offline"><div class="computer-icon"><i class="bi bi-pc-display-horizontal"></i></div><div><h3>{{ computer.server }}</h3><p><i class="bi bi-hdd-network"></i> {{ computer.ip }}</p></div><span class="status-badge">{{ 'monitor.offlineStatus' | translate }}</span></article>\r
            }\r
          </div>\r
        </details>\r
      }\r
...
//# sourceMappingURL=chunk-3DD37WA6.mjs.map
